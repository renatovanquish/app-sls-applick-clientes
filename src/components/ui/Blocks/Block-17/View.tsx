    ...
    if (isMounted) {
      ...
      if (block && block.content) {
        ...
        const fetchProductsByTags = async () => {
          const prds = [] as any
          const { items, nextToken } = await listProducts({ limit: 1000 })
          items.map((i: any) => {
            if (i.tags.indexOf(contentParse.tags) !== -1) {
              prds.push(i)
            }
          })

          if (nextToken) {
            const r = await listProducts({ limit: 1000, nextToken })
            r.items.map((i2: any) => {
              if (i2.tags.indexOf(contentParse.tags) !== -1) {
                prds.push(i2)
              }
            })
            if (r.nextToken) {
              const r2 = await listProducts({
                limit: 1000,
                nextToken: r.nextToken,
              })
              r2.items.map((i3: any) => {
                if (i3.tags.indexOf(contentParse.tags) !== -1) {
                  prds.push(i3)
                }
              })
              if (r2.nextToken) {
                const r3 = await listProducts({
                  limit: 1000,
                  nextToken: r2.nextToken,
                })
                r3.items.map((i4: any) => {
                  if (i4.tags.indexOf(contentParse.tags) !== -1) {
                    prds.push(i4)
                  }
                })
                setProducts(prds)
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {item.kitItems &&
          item.kitItems.items &&
          item.kitItems.items.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {item.kitItems.items.map((i2: any, k2: number) => (
                <div
                  className="badge badge-info text-white line-clamp-1"
                  key={k2}
                >
                  {i2.product.name}
                </div>
        ...
        <div className="mt-3 text-xl font-medium text-red">
          {item.price_of > 0 && (
            <span>
              <span>De&nbsp;</span>
      ...
      >
        ...
        {(!content.showDescription ||
          (content.showDescription !== 'false' && item.description)) && (
            <p
              className={cn(
                `text-accent-5 ${!content.textSize || content.textSize === 'default'
                  ? 'text-sm'
                  : content.textSize === 'lg'
                    ? 'text-base'
                    : 'text-sm'
        ...
        <div
          className={cn('font-medium text-red', {
                        ...
                        {options
                          ...
                          .map((option: any, index: number) => (
                            <tr key={`opt${index}`}>
                              ...
                              <td>
                                <div
                                  className={`text-accent-7 ${isSm && 'font-semibold'
                                    }`}
                                >
                                  {option.name}
                                </div>
