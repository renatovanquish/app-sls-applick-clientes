...
export default function View(props: Props) {
  ...
  return (
    ...
    >
      <div className="mt-10 mb-20 mx-auto flex sm:flex-nowrap flex-wrap">
        <div
          ...
          <div className="bg-accent-0 relative flex flex-wrap py-6 rounded shadow-md w-full">
            {content.address && (
              <div className="lg:w-1/2 px-6">
                <h2 className="title-font font-semibold text-accent-9 tracking-widest text-xs">
                  ENDEREÇO
                </h2>
                <p className="mt-1 text-accent-6">{content.address}</p>
              </div>
            )}
            <div className="w-full lg:w-1/2 px-6 mt-4 lg:mt-0">
              ...
              {content.email && (
                <a className="text-tertiaty-2 leading-relaxed text-accent-6">
                  {content.email}
                </a>
              )}
              {(content.phone1 || content.phone2) && (
                <h2 className="title-font font-semibold text-accent-9 tracking-widest text-xs mt-4">
                  TELEFONE
                </h2>
              )}
              {content.phone1 && (
        ...
        <div className="py-4 px-4 rounded shadow-md lg:w-1/3 md:w-1/2 bg-accent-0 flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0">
          <form onSubmit={handleSubmit(onSubmit)}>
            <h2 className="text-accent-9 text-lg mb-1 font-medium title-font">
              {content.blockTitle}
            </h2>
            ...
            {content.notes && (
              <p className="text-xs text-gray-500 mt-3">{content.notes}</p>
            )}
          </form>
        </div>
      </div>
