    ...
    <div className={`flex flex-col items-center grow overflow-hidden`}>
        ...
        {messages.length > 0 ? (
          [...messages]
            ...
            .map((m, i) => {
              ...
              return (m.role === "system" ? <IntermediateStep key={m.id} message={m}></IntermediateStep> : <ChatMessageBubble key={m.id} message={m} sources={sourcesForMessages[sourceKey]}></ChatMessageBubble>)
            })
        ) : (
          ""
        )}
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          <input
            className="grow mr-6 p-3 rounded"
            value={input}
            placeholder={placeholder ?? "Como posso ajudar?"}
            onChange={handleInputChange}
          />
          <button type="submit" className="shrink-0 px-6 py-3 bg-tertiary text-tertiary font-semibold rounded-lg w-28">
