  ...
  const handleForgotPassword = async (e: React.SyntheticEvent<EventTarget>) => {
    ...
    forgotPassword(loginInput.trim()).then((r: any) => {
      ...
      switch (r && r.data && r.data.message) {
        ...
        case 'tryForgotPasswordEmail':
          setMessage(
            'Caso já tenha se cadastrado, tente recuperar sua senha informando seu email. Caso contrário retorne ao login e clique em cadastre-se.'
          )
          setLoading(false)
          break

        case 'tryForgotPasswordPhone':
          setMessage(
            'Caso já tenha se cadastrado, tente recuperar sua senha informando seu celular com ddd. Caso contrário retorne ao login e clique em cadastre-se.'
          )
          setLoading(false)
          break
      ...
      {step === 1 && (
        ...
        >
          ...
          <div className="flex flex-col space-y-4">
            ...
            {message && (
              <div className="max-w-sm whitespace-pre-line py-3 px-5 mb-4 text-rose-500 bg-accent-2 text-sm rounded-md border border-red-500">
                {message}
              </div>
            )}
      ...
      {step === 2 && (
        ...
        >
          ...
          <div className="py-6 text-center text-accent-7 text-lg font-semibold">
            {deliveryMedium === 'SMS' && (
              <span>
                Verifique seu celular {destination} e informe abaixo o código de
                verificação recebido e a nova senha.
              </span>
            )}
            {deliveryMedium !== 'SMS' && (
              <span>
