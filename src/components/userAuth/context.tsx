...
export const UserAuthProvider: FC<Props> = (props) => {
  ...
  const resendTempPassword = async (login: string) => {
    ...
    if (validate(loginFmt)) {
      ...
      if (onlyNumbers.substr(0, 2) !== '55' || onlyNumbers.length < 12) {
  ...
  const signUp = async (
    ...
    try {
      console.log('try 1 - phone')
      const cognitoUser = await Auth.signIn(phone, password)

      if (cognitoUser && cognitoUser.username) {
        if (cognitoUser.challengeName === 'NEW_PASSWORD_REQUIRED') {
          dispatch({ type: 'CHANGE_PASSWORD', login: phone, oldpass: password })
          ...
          if (user) {
            ...
            setLogUser({
              userID: user.id,
              tag: 'LOGIN',
              notes: `Conectou pelo login: ${phone}.`,
            })
          } else {
            dispatch({ type: 'FETCH_USER_DATA_FAILURE' })
          }
      ...
      if (error1.code == 'UserNotConfirmedException') {
        return error1
      }

      try {
        console.log('try 2 - email')
          ...
          if (cognitoUser.challengeName === 'NEW_PASSWORD_REQUIRED') {
            ...
            if (user) {
              ...
              setLogUser({
                userID: user.id,
                tag: 'LOGIN',
                notes: `Conectou pelo login: ${email}.`,
              })
            } else {
              dispatch({ type: 'FETCH_USER_DATA_FAILURE' })
            }
        ...
        if (error2.code == 'NotAuthorizedException') {
          return error2
        } else if (error2.code == 'UserNotConfirmedException') {
          return error2
        } else {
          console.log('signUp')
  ...
  const signIn = async (login: string, password: string) => {
    try {
      ...
      if (error2.code === 'UserNotFoundException') {
        ...
        */
        return {
          message: loginMethod === 'EMAIL' ? 'tryLoginPhone' : 'tryLoginEmail',
        }
      }
  ...
  const confirmSignUp = async (login: string, code: string) => {
    try {
      ...
      console.log(loginFmt, code)

      const r = await Auth.confirmSignUp(loginFmt, code)
      if (r === 'SUCCESS') {
        const cognitoUser = await Auth.signIn(loginFmt, state.tempPass)

        const { attributes } = cognitoUser
        const { sub, name, email, phone_number } = attributes
  ...
  const resendSignUp = async (login: string) => {
    return Auth.resendSignUp(login)
      ...
      .catch(async (error) => {
        ...
        return 'Houve um erro ao reenviar o código.'
        /*
        let login2: any
        if (loginMethod === 'EMAIL') {
          try {
          ...
          */
          return {
            data: {
              message:
                loginMethod === 'EMAIL'
                  ? 'tryForgotPasswordPhone'
                  : 'tryForgotPasswordEmail',
            },
          }
  ...
  const changePasswordRequired = async (
    ...
    if (CognitoUser && CognitoUser.challengeName === 'NEW_PASSWORD_REQUIRED') {
      ...
      const cognitoUser2 = await Auth.completeNewPassword(
        CognitoUser,
        newPassword
        // requiredAttributes
      )
  ...
  const getUser = async (variables: GetUserQueryVariables) => {
    const {
      ...
      query: customQueries.getUserCustom,
      variables,
      authMode: GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS,
    })) as GraphQLResult<any>
    return getUser
  ...
  const getProfile = async (userID: string) => {
    try {
      ...
      if (
        ...
        const input: Profile = {
          userID,
          doc: getTempAux && getTempAux.doc ? getTempAux.doc : null,
          docType: null,
          specialties: null,
          subSpecialties: null,
          profession: null,
          bio: null,
          gender: GenderOptions.UNKNOWN,
          birth: getTempAux && getTempAux.birth ? getTempAux.birth : null,
          notes: null,
          urlUserName: null,
          urlEnable: null,
          allowViewEmail: true,
  ...
  const updateUserAddresses = async (input: any) => {
    ...
    try {
      ...
      setLogUser({
        userID: user.id,
        tag: 'Erro - ATUALIZAÇÃO DADOS PESSOAIS',
        notes: 'Cadastramento de endereço.',
        message: JSON.stringify({ error, input }),
      })
...
async function checkUserAuth(cognitoUser: any, setLogUser: any) {
  try {
    ...
    if (getUser) {
        ...
        const {
          ...
          variables: {
            input: {
              ...
              search: cognitoUser.attributes.name.toLowerCase(),
              active: true,
            },
          },
          authMode: GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS,
      ...
      await API.graphql({
        query: mutations.deleteTempAux,
        variables: { input: { id: cognitoUser.username } },
        authMode: GRAPHQL_AUTH_MODE.API_KEY,
      })
...
async function handleUserAuth(user: any, cognitoUser: any) {
  ...
  user.isProfissional = user.groups.includes('Profissional')
  user.isClinica = user.groups.includes('Clinica')
