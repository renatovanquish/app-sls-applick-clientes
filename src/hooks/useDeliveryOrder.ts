...
import {
  ...
  ListDeliveryByCompanyDateQueryVariables,
...
export const useDeliveryOrder = () => {
  ...
  const listDeliveryByCompanyDate = async (
    variables: ListDeliveryByCompanyDateQueryVariables
  ) => {
    const {
      data: {
        listDeliveryByCompanyDate: { items, nextToken },
      },
    } = (await API.graphql({
      query: customQueries.listDeliveryByCompanyDateCustom,
      variables,
      authMode: GRAPHQL_AUTH_MODE.API_KEY,
    })) as GraphQLResult<any>
