...
export const usePagarme = () => {
  const getCustomerIDPagarme = async (user: any) => {
    ...
    if (user && user.profile && user.profile.customerPagarmeID) {
      return user.profile.customerPagarmeID
    }
    ...
    const body = {
      ...
      email: user.email,
      code: user.id,
      document: user.profile && user.profile.doc ? user.profile.doc : null,
      type: 'individual',
      document_type:
        user.profile && user.profile.docType ? user.profile.docType : null,
      phones: {
        mobile_phone: {
          country_code,
          number,
