...
import {
  ...
  GetProfileByDocQueryVariables,
  ListUsersQueryVariables,
  CreateUserInput,
  ListProfilesByCompanyQueryVariables,
  UpdateProfileInput,
  ...
  UpdateUserInput,
  AdminUpdateUserAttributesMutationVariables,
} from 'API'
...
export const useUser = () => {
  ...
  const getProfileByDoc = async (variables: GetProfileByDocQueryVariables) => {
    const {
      data: {
        getProfileByDoc: { items, nextToken },
      },
    } = (await API.graphql({
      query: customQueries.getProfileByDocCustom,
      variables,
      authMode: GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS,
    })) as GraphQLResult<any>
    return items[0]
  ...
  const listProfilesByCompany = async (
    variables: ListProfilesByCompanyQueryVariables
  ) => {
    const {
      data: {
        listProfilesByCompany: { items, nextToken },
      },
    } = (await API.graphql({
      query: customQueries.listProfilesByCompanyCustom,
      variables,
      authMode: GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS,
    })) as GraphQLResult<any>
    return { items, nextToken }
  ...
  const userExists = async (input: UserExists) => {
  ...
  const adminUpdateUser = async (input: AdminCreateUserMutationVariables) => {
  ...
  const adminUpdateUserAttributes = async (
    variables: AdminUpdateUserAttributesMutationVariables
  ) => {
    const {
      data: { adminUpdateUserAttributes },
    } = (await API.graphql({
      query: mutations.adminUpdateUserAttributes,
      variables,
      authMode: GRAPHQL_AUTH_MODE.API_KEY,
    })) as GraphQLResult<any>
