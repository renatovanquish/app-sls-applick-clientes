...
import {
  ...
  ListKitItemsByProductQueryVariables,
  CreateKitItemInput,
  DeleteKitItemInput,
} from 'API'
...
export const useProduct = () => {
  ...
  const listProductsByCategorySubCategoryName = async (
    ...
    const {
      ...
      graphqlOperation(
        customQueries.listProductsByCategorySubCategoryNameCustom,
        variables
      )
    )) as GraphQLResult<any>
  ...
  const listKitItemsByProduct = async (
    variables: ListKitItemsByProductQueryVariables
  ) => {
    const {
      data: {
        listKitItemsByProduct: { items, nextToken },
      },
    } = (await API.graphql({
      query: queries.listKitItemsByProduct,
      variables,
      authMode: GRAPHQL_AUTH_MODE.API_KEY,
    })) as GraphQLResult<any>
    return { items, nextToken }
  ...
  const createKitItem = async (input: CreateKitItemInput) => {
    try {
      return await API.graphql({
        query: mutations.createKitItem,
        variables: { input },
        authMode: GRAPHQL_AUTH_MODE.AMAZON_COGNITO_USER_POOLS,
      })
    } catch (r: any) {
  ...
  const deleteKitItem = async (input: DeleteKitItemInput) => {
    try {
      await API.graphql(graphqlOperation(mutations.deleteKitItem, { input }))
    } catch (r: any) {
      const message =
        r && r.errors[0] && r.errors[0].message ? r.errors[0].message : null
      if (message) {
