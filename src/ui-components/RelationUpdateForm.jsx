...
import { getRelation } from "../graphql/queries";
import { updateRelation } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function RelationUpdateForm(props) {
  ...
  const [relationRecord, setRelationRecord] = React.useState(relationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getRelation.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRelation
        : relationModelProp;
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateRelation.replaceAll("__typename", ""),
            variables: {
              input: {
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || relationModelProp)}
