...
import { getAuthorizationListClosure } from "../graphql/queries";
import { updateAuthorizationListClosure } from "../graphql/mutations";
export default function AuthorizationListClosureUpdateForm(props) {
  const {
    id: idProp,
    authorizationListClosure: authorizationListClosureModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = authorizationListClosureRecord
      ? { ...initialValues, ...authorizationListClosureRecord }
      : initialValues;
    ...
    setProfessionalID(cleanValues.professionalID);
    setOS(cleanValues.OS);
    setCreatedAt(cleanValues.createdAt);
    setErrors({});
  };
  const [authorizationListClosureRecord, setAuthorizationListClosureRecord] =
    React.useState(authorizationListClosureModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getAuthorizationListClosure.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAuthorizationListClosure
        : authorizationListClosureModelProp;
      setAuthorizationListClosureRecord(record);
    };
    queryData();
  }, [idProp, authorizationListClosureModelProp]);
  React.useEffect(resetStateValues, [authorizationListClosureRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateAuthorizationListClosure.replaceAll("__typename", ""),
            variables: {
              input: {
                id: authorizationListClosureRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AuthorizationListClosureUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || authorizationListClosureModelProp)}
