...
import { getVaccinationCard } from "../graphql/queries";
import { updateVaccinationCard } from "../graphql/mutations";
export default function VaccinationCardUpdateForm(props) {
  const {
    id: idProp,
    vaccinationCard: vaccinationCardModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = vaccinationCardRecord
      ? { ...initialValues, ...vaccinationCardRecord }
      : initialValues;
    ...
    setIsOwner(cleanValues.isOwner);
    setErrors({});
  };
  const [vaccinationCardRecord, setVaccinationCardRecord] = React.useState(
    vaccinationCardModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getVaccinationCard.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getVaccinationCard
        : vaccinationCardModelProp;
      setVaccinationCardRecord(record);
    };
    queryData();
  }, [idProp, vaccinationCardModelProp]);
  React.useEffect(resetStateValues, [vaccinationCardRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateVaccinationCard.replaceAll("__typename", ""),
            variables: {
              input: {
                id: vaccinationCardRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "VaccinationCardUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || vaccinationCardModelProp)}
