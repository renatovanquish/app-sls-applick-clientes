...
export default function CampaignUpdateForm(props) {
  const {
    id: idProp,
    campaign: campaignModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = campaignRecord
      ? { ...initialValues, ...campaignRecord }
      : initialValues;
    ...
    setDiscountValue(cleanValues.discountValue);
    setQtyLimit(cleanValues.qtyLimit);
    setQtyUsed(cleanValues.qtyUsed);
    setQtyProduct(cleanValues.qtyProduct);
    setQtyProductUsed(cleanValues.qtyProductUsed);
    setOrientation(cleanValues.orientation);
    setOrderMessage(cleanValues.orderMessage);
    setZipCodeCoverage(cleanValues.zipCodeCoverage ?? []);
    setCurrentZipCodeCoverageValue("");
    setSearch(cleanValues.search);
    setErrors({});
  };
  const [campaignRecord, setCampaignRecord] = React.useState(campaignModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getCampaign.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCampaign
        : campaignModelProp;
    ...
    queryData();
  }, [idProp, campaignModelProp]);
  React.useEffect(resetStateValues, [campaignRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          qtyLimit: qtyLimit ?? null,
          qtyUsed: qtyUsed ?? null,
          qtyProduct: qtyProduct ?? null,
          qtyProductUsed: qtyProductUsed ?? null,
          orientation: orientation ?? null,
          orderMessage: orderMessage ?? null,
          zipCodeCoverage: zipCodeCoverage ?? null,
          search: search ?? null,
        };
      ...
      {...getOverrideProps(overrides, "CampaignUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || campaignModelProp)}
